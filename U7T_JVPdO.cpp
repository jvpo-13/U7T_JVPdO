...
void loop_display(){
  ...
  if (intensity > max_peak) max_peak = intensity;
  find_led(intensity);
  ...
  if (intensity >= 85.0f) elapsed85 += dt;
  if (intensity >= 88.0f) elapsed88 += dt;
  if (intensity >= 91.0f) elapsed91 += dt;
  if (intensity >= 94.0f) elapsed94 += dt;
  if (intensity >= 97.0f) elapsed97 += dt;
  ...
  if (!alarmActive) {
    ...
    else if (elapsed97 >= safe97) {
      alarmCountSafe++;
      triggerAlarm("Temp Expos 97dB");
      //           "Ultimo  Motivo:"
      alarmActive = true;
    }
    else if (elapsed94 >= safe94) {
      alarmCountSafe++;
      triggerAlarm("Temp Expos 94dB");
      alarmActive = true;
    }
    else if (elapsed91 >= safe91) {
      alarmCountSafe++;
      triggerAlarm("Temp Expos 91dB");
      alarmActive = true;
    }
    else if (elapsed88 >= safe88) {
      alarmCountSafe++;
      triggerAlarm("Temp Expos 88dB");
      alarmActive = true;
    }
    else if (elapsed85 >= safe85) {
      alarmCountSafe++;
  ...
  switch(page) {
    ...
    case 2: {
      ...
      for (int i = 0; i < NUM_READINGS; i++) {
        if (mic_readings[i] > peak) peak = mic_readings[i];
        mean += mic_readings[i];
      }
      mean /= NUM_READINGS;
